...
def build():
    ...
    for cert in certs:
        entry = providers.setdefault(cert["provider"], {
            "name": cert["provider_name"], "count": 0, "path": f"{EXAMS}/{cert['provider']}/"})
        entry["count"] += 1

    tracks = sum(1 for c in certs if c["provider"] == "anthropic")
    return {
        "_comment": "Generated by .github/scripts/build-certs-index.py. Do not edit by hand.",
        "totals": {
            "cert_directories": len(certs),
            "certifications": len(certs) - tracks,
            "study_tracks": tracks,
            "certification_providers": len([p for p in providers if p != "anthropic"]),
...
def report(index):
    ...
    print("\nStatus:")
    for status in ("active", "outline", "retired", "anticipated"):
        hits = [c["id"] for c in certs if c["status"] == status]
        print(f"  {status:<12} {len(hits):3d}")
        if status in ("outline", "retired", "anticipated"):
            for h in hits:
                print(f"      {h}")
