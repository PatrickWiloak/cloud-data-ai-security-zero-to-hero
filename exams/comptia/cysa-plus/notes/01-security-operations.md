...
happening, then place that activity inside a recognized attack framework.
...
- **Zero trust** - never trust based on network location alone; authenticate and authorize every request. See [zero trust architecture](../../../../resources/architecture-patterns/zero-trust-architecture.md).
- **Defense in depth** - layered controls so no single failure is fatal.
- **Asset inventory** - the list of what you own. You cannot protect or prioritize what you do not know exists, which is why inventory underpins vulnerability management too.
...
- **Log aggregation** - centralizing logs so an attacker who wipes a host cannot erase the evidence.
- **Time synchronization** - NTP across all sources. Without it, correlating events across systems is guesswork.
...
- **Memory consumption and unauthorized processes** - unfamiliar binaries, or familiar names in wrong locations.
- **Data exfiltration** - large archives staged before transfer, often compressed and encrypted.
- **Abnormal OS process behavior** - a process spawning an unexpected child, for example a document application launching a shell. Very high signal.
- **File system changes** - new executables in temp directories, modified system binaries.
- **Registry changes and unauthorized scheduled tasks** - classic persistence.
...
| Sandboxing | Detonation environments | Observing behavior without risking production |
...
- **Sandboxing** - executing suspect code in an isolated environment to observe behavior.
- **DKIM, SPF, and DMARC** - email authentication mechanisms. SPF authorizes sending IPs, DKIM signs the message, DMARC ties them together and states a policy.
...
- **Cyber Kill Chain** - Lockheed Martin's linear model: reconnaissance, weaponization, delivery, exploitation, installation, command and control, actions on objectives.
...
- **Indicator of compromise (IoC)** - an artifact observed that suggests an intrusion: a hash, an IP, a domain, a registry key.
- **Tactics, techniques, and procedures (TTPs)** - the behavioral patterns of an adversary. Harder for an attacker to change than an IoC, so more durable for detection.
...
- **Information sharing and analysis centers (ISACs)** - sector-specific bodies for sharing threat data.
...
- **Standardization of processes** - documented, repeatable procedures so outcomes do not depend on which analyst is on shift.
- **Streamlining operations** - automating repetitive tasks, typically enrichment and triage, so analysts spend time on judgment.
...
- If asked what best survives an attacker's changes, the answer relates to TTPs or behavior, not to a specific IoC.
